...
export const ProfilePage: React.FC = () => {
  ...
  return (
    <div className="max-w-4xl space-y-8">
      ...
      <section className="rounded-2xl border border-slate-800 bg-[#111827] p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-base font-semibold text-white">Session</h2>
          <p className="mt-1 text-sm text-slate-400">Sign out of NUVYRA on this device. Your account and saved data will not be deleted.</p>
        </div>
        <button type="button" onClick={() => { logout(); navigate('/login', { replace: true }); }} className="shrink-0 rounded-xl border border-rose-500/30 bg-rose-500/10 px-5 py-2.5 text-sm font-semibold text-rose-300 hover:bg-rose-500/20 transition">Log out</button>
      </section>

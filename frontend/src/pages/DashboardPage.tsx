...
const Info: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
  ...
  return (
    <div className="relative inline-block">
      ...
      {open ? (
        <div className="absolute z-30 right-0 top-8 w-72 rounded-2xl border border-slate-700 bg-slate-950 p-4 shadow-2xl text-left">
    ...
    <div className="rounded-3xl bg-[#111827] border border-slate-800 p-5 hover:border-slate-700 transition">
      ...
      <div className="mt-5">
        ...
        <div className="flex justify-between text-[10px] text-slate-600 mt-2">
          <span>Your lower range</span><span>Your usual range</span><span>Your higher range</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-8 pb-8">
      ...
      <div className="rounded-3xl bg-gradient-to-br from-sky-950/50 via-[#111b2d] to-[#0d1523] border border-sky-500/20 p-6 sm:p-8 shadow-xl">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-xl font-semibold text-white">How familiar is today?</h2>
              <Info title="Your personal pattern">NUVYRA learns your usual pattern from previous check-ins. It does not compare you with a single universal healthy number.</Info>
            </div>
        ...
        <div className="mt-8 grid lg:grid-cols-3 gap-6 items-center">
          <div>
            ...
            <div className="text-6xl font-extrabold text-white mt-1">{latest ? Math.round(score) : '—'}</div>
            <p className="text-xs text-slate-400 mt-2">A combined observational score, not a medical diagnosis.</p>
          </div>
      ...
      <section>
        <div className="flex items-end justify-between mb-4">
          <div><h2 className="text-xl font-bold text-white">Your signals</h2><p className="text-sm text-slate-400 mt-1">Every card is one piece of your personal pattern.</p></div>
          <Link to="/biomarkers" className="text-xs text-sky-400">Explore details →</Link>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {featureCards.map((feature, index) => <FeatureCardView key={`${feature.name}-${index}`} feature={feature} />)}
        </div>
      </section>

      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 rounded-3xl bg-[#111827] border border-slate-800 p-6">
          <div className="flex items-center justify-between"><div><h2 className="text-lg font-bold text-white">Your journey</h2><p className="text-xs text-slate-400 mt-1">See how your combined pattern changes over time.</p></div><Info title="Why trends matter">A single different day is not enough to establish a meaningful pattern. NUVYRA looks at repeated observations and their quality over time.</Info></div>
          <div className="mt-5"><TrendChart data={trendData} label="Your pattern over time" /></div>
        </div>
        <div className="rounded-3xl bg-[#111827] border border-slate-800 p-6">
          <div className="flex items-center gap-2"><h2 className="text-lg font-bold text-white">What NUVYRA noticed</h2><Info title="AI interpretation">The AI combines available signals, checks data quality and looks for changes from your personal history. It should not be treated as a diagnosis.</Info></div>
          <p className="text-sm text-slate-300 mt-4 leading-7">{latest?.explanation ?? 'Your first check-in will give NUVYRA the information it needs to start building your personal reference.'}</p>
          <div className="mt-5 pt-4 border-t border-slate-800 space-y-2 text-xs"><div className="flex justify-between"><span className="text-slate-500">Information clarity</span><span className="text-slate-200">{latest ? `${Math.round(latest.confidence * 100)}%` : 'Not available yet'}</span></div><div className="flex justify-between"><span className="text-slate-500">Previous check-ins</span><span className="text-slate-200">{latest?.baseline_observations ?? history?.baseline_observations ?? 0}</span></div></div>
        </div>
      </div>

      <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-5"><h3 className="text-sm font-semibold text-white">ⓘ How NUVYRA understands your data</h3><p className="text-xs leading-6 text-slate-400 mt-2">There is no single number that is ideal for every person. NUVYRA learns your own usual pattern, checks the quality of today's information, and looks for changes that persist over time. Environment, camera position, microphone quality and other everyday factors can affect measurements.</p></div>

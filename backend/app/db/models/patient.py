"""
Patient Profile and Demographic ORM Models.
"""
...
class PatientProfile(Base):
    ...
    primary_diagnosis: Mapped[str] = mapped_column(
        String(255), default="Chronic Myeloid Leukemia (CML)", nullable=False
    )
    secondary_diagnosis: Mapped[Optional[str]] = mapped_column(String(255), nullable=True)
    disease_phase: Mapped[Optional[str]] = mapped_column(
        String(50), default="Chronic Phase", nullable=True
    )
    ...
    current_disease_status: Mapped[Optional[str]] = mapped_column(
        String(100), default="Active Treatment", nullable=True
    )
    ...
    treating_physician_id: Mapped[Optional[int]] = mapped_column(
    ...
    medications: Mapped[List["MedicationRegimen"]] = relationship(
        "MedicationRegimen", back_populates="patient", cascade="all, delete-orphan"
    )
    ...
    nutrition_logs: Mapped[List["NutritionLog"]] = relationship(
        "NutritionLog", back_populates="patient", cascade="all, delete-orphan"
    )
    lifestyle_logs: Mapped[List["LifestyleLog"]] = relationship(
        "LifestyleLog", back_populates="patient", cascade="all, delete-orphan"
    )

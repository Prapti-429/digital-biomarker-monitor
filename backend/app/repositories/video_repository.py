...
class VideoRepository(BaseRepository[VideoRecording, Any, Any]):
    """
    ...
    def update_metadata(self, video_id: int, metadata: Dict[str, Any]) -> VideoRecording:
        """
    ...
    def retrieve_video_timeline(self, patient_id: int) -> List[VideoRecording]:
        """
        ...
        try:
            stmt = (
                ...
                .join(DailyCheckIn, VideoRecording.check_in_id == DailyCheckIn.id)
                .where(DailyCheckIn.patient_id == patient_id)
                .order_by(VideoRecording.created_at.asc())
            )

...
class AIService:
    ...
    def _model_score(self, current: Dict[str, float], history_features: List[Dict[str, float]]):
        ...
        if n >= 5 and len(current) >= 4:
            try:
                ...
                pass
        if not deviations: return 82.0, "robust_baseline_initialization", 0.45, deviations
    ...
    @staticmethod
    def _drivers(deviations): return [name.replace("_", " ") for name, value in sorted(deviations.items(), key=lambda x: x[1], reverse=True)[:4] if value >= 1]
    @staticmethod
    def _missing(modalities): return [m for m in MODALITY_NAMES if m not in modalities]

    def _recommendations(self, score, trend, quality, missing, context):
        ...
        if context.pending_reminder_count: out.append("You have pending health-document reminders. Review Notifications and follow your clinician's instructions.")
    ...
    def analyze(self, user_id: UUID, payload: AIAnalysisRequest) -> AIAnalysisResponse:
        ...
        recommendations = self._recommendations(score, trend, quality, missing, context); limitations = self._limitations(len(history), missing, context)
        now = datetime.now(timezone.utc)
        explanation = f"The current session is {('broadly consistent' if score >= 80 else 'moderately different' if score >= 60 else 'substantially different')} from the personal baseline. Trend: {trend}. Persistence: {persistence}. The assessment used {len(modalities)} available signal groups. Past history ({context.past_history_count} entries), uploaded documents ({context.document_count}), and pending reminders ({context.pending_reminder_count}) are included as contextual information, not as diagnostic evidence. This is an observational signal, not a diagnosis."

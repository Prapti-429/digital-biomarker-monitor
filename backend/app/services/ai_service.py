...
class AIService:
    ...
    def _safe(value: Optional[float]) -> Optional[float]:
        try:
            if value is None or not isfinite(float(value)): return None
            value = float(value)
            # Reject impossible/non-useful numeric payloads instead of poisoning the baseline.
            if abs(value) > 1e9: return None
            return value
    ...
    def _quality(self, current: Dict[str,float], payload: AIAnalysisRequest) -> float:
        ...
        present_modalities = len(self._modalities(current)); modality_coverage = present_modalities/len(MODALITY_NAMES)
        finite_ratio = len(current)/len(FEATURES)
        duration_quality = min(1.0, max(0.0, float(payload.source_duration_seconds or 0))/20.0)
        # A modality contributes only when at least one finite feature exists.
    ...
    def _model_score(self, current: Dict[str,float], history_features: List[Dict[str,float]):
        ...
        if not deviations: return (82.0 if current else 0.0), "baseline_initialization", min(0.7, 0.30+n*0.04), deviations
    ...
    @staticmethod
    def _modalities(current: Dict[str,float]) -> List[str]: return sorted({category for name,category in FEATURES if name in current})

    @staticmethod
    def _missing(modalities: List[str]) -> List[str]: return [m for m in MODALITY_NAMES if m not in modalities]
    ...
    def _persistence(self, current_deviations: Dict[str,float], history: List[Tuple]) -> str:
        ...
        # Compare the same feature against its own historical median for each of the latest sessions.
        recent=history[:3]
        persistent=[]
        for name in notable:
            ...
            center,_=self._median_mad(all_hist)
            scale=self._median_mad(all_hist)[1]
            if sum(abs(v-center)/scale>=1 for v in values)>=2: persistent.append(name)
        return "PERSISTENT_CHANGE" if persistent else "SINGLE_SESSION_CHANGE"

    @staticmethod
    def _drivers(deviations: Dict[str,float]): return [n.replace("_"," ") for n,v in sorted(deviations.items(),key=lambda x:x[1],reverse=True)[:4] if v>=1]
    ...
    def _recommendations(self, score, trend, quality, missing, context, persistence):
        ...
        if quality<0.6: out.append("Today's information is limited or noisy. A cleaner recording or additional signal groups may improve the next check-in.")
        if missing: out.append("Missing signal groups were excluded from the analysis; they were not treated as normal.")
        if persistence=="PERSISTENT_CHANGE": out.append("A change has appeared repeatedly in recent usable observations. If it concerns you, discuss it with a qualified healthcare professional.")
        if context.pending_reminder_count: out.append("You have pending document reminders. Review Notifications and follow your clinician's instructions.")
        return out
    ...
    def _limitations(history_count, missing, quality):
        ...
        if history_count<5: out.append("Your personal baseline is still developing; confidence is limited until more usable observations are collected.")
        if missing: out.append("Unavailable modalities were excluded rather than imputed as normal.")
        if quality<0.6: out.append("Today's data quality was limited, so interpretation should be treated cautiously.")
        return out

    def analyze(self,user_id: UUID,payload: AIAnalysisRequest)->AIAnalysisResponse:
        current=self._vector(payload); history=self._history(user_id); context=self._context(user_id)
        score,algorithm,confidence,deviations=self._model_score(current,[r[2] for r in history])
        previous=history[0][1].overall_score if history else None; trend=self._trend(score,previous)
        modalities=self._modalities(current); missing=self._missing(modalities); quality=self._quality(current,payload)
        persistence=self._persistence(deviations,history); drivers=self._drivers(deviations)
        recommendations=self._recommendations(score,trend,quality,missing,context,persistence); limitations=self._limitations(len(history),missing,quality)
        ...
        self.db.add(check_in); self.db.flush(); feature_reads=[]
        for name,category in FEATURES:
            if name in current:
                value=current[name]; self.db.add(BiomarkerFeature(check_in_id=check_in.id,feature_name=name,feature_category=category,feature_value=value,source_modality=category,extracted_at=now,extra_properties={"model_version":MODEL_VERSION,"research_proxy":category in {"breathing","facial_dynamics","eye","gait_movement","head_movement"}})); feature_reads.append(BiomarkerFeatureRead(name=name,category=category,value=value,deviation=deviations.get(name)))
        ...
        self.db.add(HealthStabilityScore(check_in_id=check_in.id,overall_score=round(score,2),trend_category=trend,confidence=round(confidence,3),generated_at=now,explanation_summary=explanation,model_metadata=metadata)); self.db.commit()
    ...
    def latest(self,user_id: UUID):
        ...
        c,s,_=rows[0]; features=self.db.execute(select(BiomarkerFeature).where(BiomarkerFeature.check_in_id==c.id)).scalars().all(); m=s.model_metadata or {}
